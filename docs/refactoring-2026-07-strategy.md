...
- [ ] res.status().json() 直書きが utils/errors.js 以外で 0
- [ ] 無視パターン定義が 1 箇所
- [ ] escapeHtml 実装がサーバー側 1 箇所
- [ ] 全ミューテーション API に Origin ガード
- [ ] POST /api/file が atomicWrite + withPathLock 経由
- [ ] app.js が 300 行以下の bootstrap になり、modules/ が各 500 行以下
- [ ] フロント HTTP 呼び出しが apiClient 経由に統一（XHR upload は文書化された例外）
- [ ] E2E スモーク 10 本 + CI が回る
- [ ] bin/mdv.js のロジックが src/cli/ で export されユニットテスト可能
