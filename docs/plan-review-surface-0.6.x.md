...
| 0.6.2 | changeJournal + lineDiff + /api/diff + file_update への hash 追加（バックエンド完結） | 同上 |
| 0.6.3 | 差分バー+ハイライト+ジャンプ（フロント） | 同上 + 実機目視 |
| 0.6.4 | 未読●/✓チェック・フォルダバッジ・次の未読へ | 同上 + 実機目視 |
| 0.6.5 | 統合磨き: 大量フォルダでの性能確認（500件cap環境）・E2E拡充・README/CHANGELOG | 同上 |
| 0.6.6-0.6.9 | ドッグフード/codex 指摘・予備枠（Windows対応検討・presenterテスト等の残債は任意） | — |
