...
**校閲（Review）ボタン1つで全部ON/OFF**（0.6.12）— ツールバーの「Review」
ボタンが、未読●・変更ハイライト・削除の取り消し線・「次の変更 N」/「✓ 確認」
まで、レビュー関連の表示をまとめて出し入れします。既定はOFF（普段は素の
プレビュー）。Wordの校閲タブと同じ考え方で、裏側の追跡（既読の記録など）
は表示ON/OFFに関係なく常に動き続けるので、ONにした瞬間から正しい状態が
見えます。「次の変更 N」「✓ 確認」は Review ボタンの右に常に並び、
対象がないタブではグレーアウトします（0.6.14 — ボタンの出没でツールバーが
ガタつかない）。
...
- 🟡 **変更ハイライト**（0.6.3〜） — 前回確認時からの差分を検出し、追加・
  変更行は黄でブロックハイライト、削除行は該当箇所にWordの取り消し線の
  ように削除内容そのものをインライン表示、`⌥↑↓` で変更箇所へジャンプ。
  差分のあるタブでだけツールバーに「変更 N」（クリックで次の変更へ
  ジャンプ）と「✓ 確認」の2ボタンが現れる（0.6.8 — Wordの変更履歴と同じ
  操作感を狙い、専用の3列目バーは廃止。0.6.12 — 表示そのものが上の
  Reviewボタンの ON/OFF に統合され、ONの間はハイライト/取り消し線が
  常時表示）。
- ✅ **未読● ツリーバッジ**（0.6.5〜） — 外部で変更/新規作成されたファイルに
  未読●。フォルダ行に未読数バッジ、サイドバーヘッダに合計チップ、チップ
  クリック / `⌥⇧↓` で「次の未読へ」ジャンプ（0.6.8 で確認済みの緑✓表示は
  廃止 — 未読は●の有無だけで判断します。0.6.12 — こちらもReviewボタンが
  ONのときだけ表示）。
...
```
$ mdv -p 8642
ポート 8642 は使用中のため、8643 で起動します
MDV server running at http://localhost:8643

  MDV - Markdown Viewer with Marp support

  Server running at: http://localhost:8643
  Root directory: <cwd>
...
> macOS では **Cmd のみ**がショートカット修飾キーです（Ctrl+K 等は OS のテキスト編集キーと衝突するため 0.6.7 で意図的に無効化）。Windows/Linux では **Ctrl のみ**です。
...
| F2 | ファイル名変更 |
| Delete | ファイル削除 |
| Option(⌥) + ↑ / ↓ | 変更箇所へジャンプ（Review ON かつ差分がある時のみ有効） |
| Option(⌥) + Shift + ↓ | 次の未読ファイルへジャンプ（Review ON時のみ有効） |
...
| `/raw/*` | GET | 生ファイル配信（Range Request対応。動画/音声ストリーミング等に使用） |
| `/api/tree` | GET | ファイルツリー取得 |
| `/api/tree/expand` | GET | ディレクトリ展開（遅延読み込み） |
| `/api/tree/page` | GET | 大きなディレクトリの追加ページ取得 |
| `/api/mkdir` | POST | ディレクトリ作成 |
| `/api/move` | POST | ファイル移動/リネーム |
| `/api/download` | GET | ファイルダウンロード |
| `/api/upload` | POST | ファイルアップロード |
| `/api/search` | GET | 全文検索 (`q` 必須, `limit` 省略可。⌘K / Ctrl+K パレット、0.6.1〜) |
| `/api/diff` | GET | 変更ハイライト用の差分取得 (`path` 必須, `from`=前回確認時のETagハッシュ。0.6.3〜) |
| `/api/pdf/export` | POST | PDF出力 |
| `/api/info` | GET | サーバー情報 |
| `/api/shutdown` | POST | Web UI の Stop server ボタンからサーバーを停止 |
