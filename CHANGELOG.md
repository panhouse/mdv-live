...
- **pin は `markSeen()`（ベースライン採用の唯一の書き込み口）の中で張る**。初見・✓確認・
  差分ゼロの自動採用など**採用経路が複数あり、個別に pin を張るのは必ず忘れる**ため、
  関所を1つにして構造的に取りこぼしを防いだ。フォルダ一括確認だけは明示的に pin を降りる
  （数千ファイルぶんの `/api/diff` が飛ぶため。ただし編集中のアクティブファイルは pin する）
- pin は**パスにつき直近3件の window**（`JOURNAL_PIN_HISTORY_SIZE`）。単一スロットだと、
  新しい版を確認した直後に**飛行中の古いリクエスト**が届いて pin を巻き戻し、同じ症状が再発する
